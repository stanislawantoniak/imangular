...
public class UserControllerAllGetsTests {
	...
	public void testGetUserList() throws Exception {
		 ...
		 * 
		 * UserForm uf1 = UserControllerUpdateUserTests.getUserForm("a123456"); UserForm
		 * uf2 = UserControllerUpdateUserTests.getUserForm("b123456"); UserForm[]
		 * ufArray = new UserForm[]{ uf1, uf2}; List<UserForm> list =
		 * Arrays.asList(ufArray);
		 * 
		 ...
		 * this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
		 * 
		 * mockMvc.perform( req ) .andExpect(status().isOk()) //.andExpect(jsonPath("$",
		 * arrayWithSize( 2 ) )) .andExpect(jsonPath("$", hasSize( 2 ) ))
	...
	public void testGetUser_WhenUserExists() throws Exception {
		...
		MockHttpServletRequestBuilder req = get("/userrest/" + id).contentType(APPLICATION_JSON_UTF8);

		// System.out.println(APPLICATION_JSON_UTF8);

		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();

		mockMvc.perform(req).andExpect(status().isOk()).andExpect(jsonPath("$.password", is(""))) // we never send pass
	...
	public void testGetUser_WhenUserDoesNotExist() throws Exception {
		...
		mockMvc.perform(req).andExpect(status().isOk()).andExpect(jsonPath("$.password", is(""))) // userform should be
																									// empty
	...
	public void testGetAllRoles() throws Exception {
		...
		MockHttpServletRequestBuilder req = get("/allRoles").contentType(APPLICATION_JSON_UTF8);
