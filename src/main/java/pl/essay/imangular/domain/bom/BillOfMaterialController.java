...
public class BillOfMaterialController extends BaseController {
	...
	public ResponseEntity<List<BomRequirementsQueryResult>> usedInItem(@PathVariable long bomId) {
		...
		return new ResponseEntity<List<BomRequirementsQueryResult>>(this.bomService.getBomRequirements(bomId),
				HttpStatus.OK);
	...
	public ResponseEntity<BillOfMaterial> getBom(@PathVariable("id") long id) {
		...
		BillOfMaterial bom = (id != 0 ? this.bomService.getEntityById(id) : new BillOfMaterial());// init bom or get
																									// from db
	...
	public ResponseEntity<Long> createBillOfMaterial(@RequestBody BillOfMaterial bom) {
		...
		// System.out.println("BillOfMaterial with name " + bom.getName() + " already
		// exist and requested create");
	...
	public ResponseEntity<Void> updateStockInBillOfMaterial(@PathVariable long idstock,
			@RequestBody BillOfMaterialInStock stock) {
