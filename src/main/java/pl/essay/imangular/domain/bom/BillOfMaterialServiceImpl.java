...
public class BillOfMaterialServiceImpl extends GenericServiceImpl<BillOfMaterial> implements BillOfMaterialService {
	 ...
	 * 
	 * @see
	 * pl.essay.imangular.service.BillOfMaterialService#updateBom(pl.essay.imangular
	...
	public BillOfMaterial calculateBom(BillOfMaterial bom) {
		...
		Item bomItem = this.itemDao.get(bom.getForItem().getId()); // get item so all lazy loaded collection are
																	// available
		for (ItemComponent ic : bomItem.getComponents()) {
			// System.out.println("starting recursion:: ");
			Item item = this.itemDao.get(ic.getComponent().getId()); // get item so all lazy loaded collection are
																		// available
	...
	private void calculateRecursive(Item item, long requiredFromParent, Map<Integer, BillOfMaterialInStock> stocks,
			Map<Integer, BillOfMaterialFlatListLine> requirementLines) {

		logger.debug(
				"calculating recursive for item component::" + item + ", required from parent = " + requiredFromParent);

		BillOfMaterialFlatListLine requirementLine;

		BillOfMaterialInStock stock = stocks.get(item.getId());

		int freeStock = (stock != null ? stock.getInStockQuantity() - stock.getConsumedStockQuantity() : 0);
		...
		logger.debug("    component:: " + item.getName() + " | requiredEffective:: " + requiredEffective
				+ " | stockToConsume:: " + freeStock);
		...
		requirementLine
				.setEffectiveRequiredQuantity(requirementLine.getEffectiveRequiredQuantity() + requiredEffective);
		...
		if (stock == null ? true : !stock.getIgnoreRequirement())
			if (item.getIsComposed() && requirementLine.getEffectiveRequiredQuantity() > 0) {
				for (ItemComponent component : item.getComponents()) {
					...
					this.calculateRecursive(component.getComponent(), requiredEffective * component.getQuantity(),
							stocks, requirementLines);
				}
			}
	}
	...
	public List<BomRequirementsQueryResult> getBomRequirements(long id) {
		...
		for (BillOfMaterialFlatListLine req : bom.getRequirementsList()) {
			...
			/*
			 * public int forItemId; public String forItemName; public Boolean
			 * forItemIsComposed;
			 * 
			 * public String forItemWhereManufactured; public String forItemOtherSources,
			 * 
			 * public int requiredQuantity; public int effectiveRequiredQuantity;
			 * 
			 * public int inStockQuantity; public long stockId;
			 * 
			 * public String stockRemarks;
			 */
			Integer forItemId = req.getForItem().getId();
			BillOfMaterialInStock stock = stocks.get(forItemId);
			BomRequirementsQueryResult responseLine = new BomRequirementsQueryResult(req.getForItem().getId(),
					req.getForItem().getName(), req.getForItem().getIsComposed(),
					req.getForItem().getWhereManufactured(), req.getForItem().getOtherSources(),
					req.getRequiredQuantity(), req.getEffectiveRequiredQuantity(),
					(stock == null ? null : stock.getInStockQuantity()), (stock == null ? null : stock.getId()),
