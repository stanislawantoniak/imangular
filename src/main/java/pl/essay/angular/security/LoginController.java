...
public class LoginController extends BaseController {
	...
	public String user(HttpSession httpSession) {
		...
		if (!this.userSession.getAuthenticated()) {// move all boms from anonymous to user (change owner on login)
			...
			this.bomService.moveBomsFromAnonymousToUser(this.userSession.getAnonymousSessionId(),
	...
	void print(HttpSession httpSession) {
		...
		// logger.trace("\thttpSession last
		// accessed::"+httpSession.getLastAccessedTime());
		// logger.trace("\thttpSession max inact
		// interv::"+httpSession.getMaxInactiveInterval());
	...
	/*
	 * this is unprotected resource used for fetching data stored in usersession for
	 * guest user
	 */
